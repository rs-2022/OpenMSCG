'''Iterative relative-entroy method (REM) for CG modeling with RMSProp

Description
-----------

The ``cgrem_rmsprop`` command the main engine that drives the REM approach. It applies the iterative approch to minimize the differences of ensemble averages of between derivatives from the reference model an\
d trial models. Therefore, this command calls the ``cgderiv`` command and an MD engine (currently only LAMMPS is supported) in loops. This particular version of cgrem utilizes the RMSProp algorithm for gradie\
nt descent.

Usage
-----

Syntax of running ``cgrem_rmsprop`` command ::
    ...
    Optional arguments:
      ...
      --correct             manually updates first spline to be greater than second spline by a value of 2. Empirical and should only be used with spline interactions
      --lr_decay            learning rate decay value
      --epsilon             small number to ensure no divide by zero
      --decay               type of learning rate decay
...
class OptimizerBuiltin:
    ...
    def __init__(self, **kwargs):
        ...
        self.maxstepsize=float(kwargs.get('maxstepsize', 1000000))
        self.correct=bool(kwargs.get('correct', False))
        self.memory = 0.9
        self.epsilon = 1e-9
        self.lr_decay = float(kwargs.get('lr_decay', 0.12))
        self.decay = str(kwargs.get('decay', 'time'))

    def run(self, params, dudl_ref, dudl_mean, dudl_var, dudl_grads, epochs):
        if dudl_mean is None:
            return params, dudl_grads

        for name, dudl_aa in dudl_ref.items():
            dudl_cg = dudl_mean[name].copy()
            var = dudl_var[name].copy()
            grad_his = dudl_grads[name].copy()
            for i in range(len(var)):
                if var[i]==0:
                    var[i]=1E+10

            grad = self.beta * (dudl_cg - dudl_aa)
            grad_avg = self.memory * grad_his + (1.0 - self.memory) * np.power(grad,2)
            ...
            if 'Bon' in name:
                ...
                self.begknots=1
                self.endknots=1
            grad[:self.begknots] = 0
            grad[(len(grad)-self.endknots):] = 0

            knots = open('%s_dudl.dat' %name, 'w')
            for i in range(len(dudl_cg)):
                knots.write('%s %s\n' %(dudl_cg[i], dudl_aa[i]))
            knots.close()
            if self.decay == 'time':
                param_update = param_prev + (self.chi / (1.0 + epochs * self.lr_decay)) * (grad / np.sqrt(grad_avg+self.epsilon))
            elif self.decay == 'exp':
                param_update = param_prev + (self.chi *  np.power(self.lr_decay, epochs)) * (grad / np.sqrt(grad_avg+self.epsilon))

            if self.correct:
                if param_update[0] < param_update[1]:
                    param_update[0] += 2.0
            params[name] = param_update
            dudl_grads[name] = grad_avg
            knots = open('%s_knots.dat' %name, 'w')
            ...
            for i in range(params[name].shape[0]):
                screen.info("=> %15.5e %15.5e %15.5e" % (param_prev[i], grad[i], params[name][i]))
...
def main(*args, **kwargs):
    ...
    group.add_argument("--cutoff", metavar='', type=float, default=25, help="maximum iterations")
    group.add_argument("--start_lin", metavar='', type=float, default=24.1, help="maximum iterations")
    ...
    md_cmds = open(args.md, "r").readlines()
    ...
    if os.path.isfile(args.restart + ".p"):
        ...
        for m in models.items:
            ...
            dudl_grads[m.name] = np.zeros(len(params[m.name]))
    # iterations
    ...
    for it in range(args.maxiter):
        ...
        params, dudl_grads = args.optimizer.run(params.copy(), dudl_ref, dudl_mean, dudl_var, dudl_grads,epochs)
        ...
        for m in models.items:
            ...
            if 'Bon' in m.name:
                tbl.compute(targets[m.name]['inc'], 25.0, targets[m.name]['inc'])
                ...
                tbl.compute(targets[m.name]['inc'], targets[m.name]['max'], targets[m.name]['inc'])
                pad = targets[m.name]['pad']
                if "L2" in pad:
                    tbl.padding_low2(targets[m.name]['min'])
                elif "L3" in pad:
                    tbl.padding_low3(targets[m.name]['min'])
                elif "L4" in pad:
                    tbl.padding_low4(targets[m.name]['min'], args.start_lin, args.cutoff, targets[m.name]['inc'])
                elif "L5" in pad:
                    tbl.padding_low5(targets[m.name]['min'])
                elif "L" in pad:
                    tbl.padding_low(targets[m.name]['min'])
                if "H" in pad:
                    tbl.padding_high(targets[m.name]['max'])
